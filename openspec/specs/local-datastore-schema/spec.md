...
Local persistence MUST provide an explicit remote-merge path that preserves server version, ownership, timestamps, and sync status rather than incrementing versions as if it were a local edit.
