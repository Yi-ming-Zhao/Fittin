...
3. Before loading the bootstrap script, inspect the full and Chromium CanvasKit entries when an existing Service Worker controls the page. Refresh only missing or incorrectly typed variants through versioned migration URLs using reload semantics, validate their response media types, and overwrite their canonical entries in `flutter-app-cache`. Service Worker manifests compare file bytes rather than response headers, and deleting CacheStorage alone can repopulate an invalid response from a still-fresh HTTP cache. The targeted overwrite bypasses both stale layers without touching localStorage, IndexedDB, credentials, preferences, or training data; a clean first visit skips the migration download.
