...
- Web v1/v2 加法迁移、两个连接的版本竞争、空值替换及跨仓库回滚共 4 项通过。
...
- 首次 Linux CI 发现撤销冲突后的原生 Isar 进程崩溃，本机复现。只读拒绝改为事务返回结果，事务结束后再抛出业务冲突；真正写入失败仍回滚。连续 20 次拒绝验证版本、训练进度及操作记录不变，最终由 CI 再确认。
