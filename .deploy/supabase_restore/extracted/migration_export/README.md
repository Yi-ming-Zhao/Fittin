...
如果你准备把这个项目部署到另一台服务器，而不是继续依赖当前机器上的本地静态服务，可以直接使用仓库根目录下的 `Dockerfile`。

当前仓库没有单独的自定义后端 API 服务；Docker 方案打包的是：

- Flutter Web release bundle
- 容器内 Caddy 静态托管
- 运行时直接连接你提供的远端 Supabase

快速示例：

```bash
docker build \
  --build-arg SUPABASE_URL=https://your-supabase-host.example.com \
...
如果你还要把 Supabase 一起迁到服务器，请看：

- `docs/server-self-hosted-deployment.md`
