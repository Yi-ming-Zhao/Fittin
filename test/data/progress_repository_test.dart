...
void main() {
  ...
  test(
    'claimLocalDataForUser migrates progress data and queues sync',
    () async {
      ...
      final metrics = await repository.fetchBodyMetrics(
        ownerUserId: 'user-123',
      );
      final photos = await repository.fetchProgressPhotos(
        ownerUserId: 'user-123',
      );
      ...
      final photoCollection = await isar.progressPhotoCollections
          ...
          .findFirst();
      final metricQueue = await isar.syncQueueCollections.getByQueueKey(
        '${SyncEntityTypes.bodyMetric}:metric-local',
      );
      final photoQueue = await isar.syncQueueCollections.getByQueueKey(
        '${SyncEntityTypes.progressPhoto}:photo-local',
      );
  ...
  test(
    'deleteBodyMetric marks soft delete and keeps sync queue entry',
    () async {
      await repository.saveBodyMetric(
        BodyMetric(
