...
class _ConflictingMetricRemoteRepository extends FakeSupabaseRemoteRepository {
  @override
  Future<void> upsertBodyMetric(BodyMetricCollection collection) async {
...
void main() {
      ...
      remoteRepository.rowsByTable['plans'] = [
        {
          ...
          'is_built_in': false,
          'raw_json':
              '{"id":"remote-template","name":"Remote Template","description":"Pulled from cloud","phases":[],"workouts":[]}',
      ...
      remoteRepository.rowsByTable['plan_instances'] = [
        {
          ...
          'current_states_json':
              '[{"workoutId":"rw1","exerciseId":"re1","exerciseName":"Bench","baseWeight":70.0,"currentStageId":"stage-1","history":[]}]',
      ...
      remoteRepository.rowsByTable['workout_logs'] = [
        {
          ...
          'raw_json':
              '{"instanceId":"remote-instance","workoutId":"rw1","workoutName":"Remote Workout","dayLabel":"Day R","completedAt":"2026-03-18T12:00:00.000Z","exercises":[]}',
  ...
  test(
    'synchronize replays queued changes after a retryable failure',
    () async {
      ...
      final queuedAfterFailure = await isar.syncQueueCollections
          .where()
          .findAll();
      ...
      final queuedAfterRetry = await isar.syncQueueCollections
          .where()
          .findAll();
      ...
      expect(
        flakyRemote.upserts.where((entry) => entry['table'] == 'body_metrics'),
        hasLength(1),
      );
  ...
  test('synchronize propagates soft deletes for body metrics', () async {
    ...
    final collection = await isar.bodyMetricCollections
        ...
        .findFirst();
    final queue = await isar.syncQueueCollections.getByQueueKey(
      '${SyncEntityTypes.bodyMetric}:metric-delete',
    );

    expect(
      remoteRepository.deletes.any(
        (entry) =>
            entry['table'] == 'body_metrics' && entry['id'] == 'metric-delete',
      ),
      isTrue,
    );
  ...
  test(
    'synchronize preserves local conflicts for newer pending progress data',
    () async {
      await progressRepository.saveBodyMetric(
        BodyMetric(
  ...
  test('server conflicts stay queued and become locally resolvable', () async {
    final conflictingRemote = _ConflictingMetricRemoteRepository();
    syncService = SyncService(
      databaseRepository: databaseRepository,
      progressRepository: progressRepository,
      remoteRepository: conflictingRemote,
      ownerUserId: 'user-123',
    );
    await progressRepository.saveBodyMetric(
      BodyMetric(
        metricId: 'metric-cas-conflict',
    ...
    final metric = await isar.bodyMetricCollections
        .filter()
        .metricIdEqualTo('metric-cas-conflict')
        .findFirst();
    final queue = await isar.syncQueueCollections.getByQueueKey(
      '${SyncEntityTypes.bodyMetric}:metric-cas-conflict',
    );
    expect(metric?.syncStatusKey, SyncStatusKeys.conflict);
    expect(queue, isNotNull);
  });

  test('remote instance hydration preserves the server version', () async {
    remoteRepository.rowsByTable['plan_instances'] = [
      {
        ..._remoteInstanceRow(id: 'remote-v7', templateId: 'template-v7'),
        'version': 7,
      },
    ];
    ...
    final instance = await databaseRepository.fetchInstance('remote-v7');
    expect(instance?.version, 7);
    expect(instance?.syncStatus, SyncStatusKeys.synced);
  });
}
